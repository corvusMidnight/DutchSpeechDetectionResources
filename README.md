...
|``` s2t ```|``` s2t ```|``` s2t ```|``` s2t ```|``` s2t ```|
| facebook/s2t-medium-mustc-multilingual-st   |     ![](https://img.shields.io/badge/model-blue)     |     "s2t-medium-mustc-multilingual-st is a Speech to Text Transformer (S2T) model trained for end-to-end Multilingual Speech Translation (ST)."            |    -      |    [HuggingFace](https://huggingface.co/facebook/s2t-medium-mustc-multilingual-st)     |
